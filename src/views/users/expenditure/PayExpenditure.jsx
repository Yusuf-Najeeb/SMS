...
import { processExpenditure } from '../../../store/apps/expenditure/asyncthunk'
...
const PayExpenditureBalance = ({ open, expenditure, togglePayModal, fetchData }) => {
  ...
  const validatePayment = (amount) => {
      ...
      if (amount > (expenditure.amount - expenditure.amountPaid)) {
  ...
  useEffect(() => {
    if (expenditure) {
      const owedAmount = Math.abs(expenditure.amountPaid - expenditure.amount)
  ...
  const submitPayForm = async (data) => {
    const payload = {id: expenditure.id, ...data}

    processExpenditure(payload).then((res)=>{
        if(res.data.success){
            reset()
            fetchData()
            togglePayModal()
  ...
  return (
    ...
    >
      <Header>
        <Typography variant='h5'> Expenditure Payment</Typography>
        <IconButton
          size='small'
        ...
        <Fragment>
          {expenditure !== null && expenditure !== undefined ? (
            ...
            <Grid container spacing={6}>
              <Grid item xs={12}>
                <Alert severity='error'>Expenditure does not exist. Please check the list of Expenditure: </Alert>
              </Grid>
            </Grid>
          )}
        </Fragment>
...
export default PayExpenditureBalance
